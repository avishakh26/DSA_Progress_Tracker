...
public final class SettingsController implements Refreshable {
    ...
    private void onResetProgress() {
        final boolean confirmed = new TypedConfirmDialog(themeManager, "Reset Progress",
                "This sets every topic and problem back to Not Started and clears your activity "
                        + "history. Your topics, problems and notes themselves are kept.",
                "yes I want to reset all my progress", "Yes I am absolutely SURE about that.",
                "Reset Everything").showAndWait().orElse(false);
        if (!confirmed) {
            return;
        }
    ...
    private void onClearAllData() {
        final boolean confirmed = new TypedConfirmDialog(themeManager, "Clear All Data",
                "This permanently deletes every topic, problem, note, goal and activity record. "
                        + "This cannot be undone.",
                "yes I want to delete all my data", "Yes I am absolutely SURE about that.",
                "Delete Everything").showAndWait().orElse(false);
        if (!confirmed) {
            return;
        }
